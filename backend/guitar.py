...
class Guitar:
    ...
    def find_range(self):
        ...
        max_fret = min(fret_guess + 4, self.frets)  # Max fret range

        # Get all notes within the fret range for each string
        fret_range = {
            string: {fret: note for fret, note in frets.items() if min_fret <= fret <= max_fret}
        ...
        return fret_range, fret_guess  # Return sliced fretboard
    ...
    def closest_notes(self, note_names):
        DP = {} # Store note cost
...
print(guitar.closest_notes(note_names))